...
class DiscretePotential(object):
    """
    ...
    def __init__(self, domain, sizes, T=None):
        ...
        self.observed_domain = np.array([])
        self.sizes = np.array(sizes)
        
        if T == None:
            # Remove the trailing ones from the sizes parameter so as
            # not to confuse the potentials.arithmatic function with
            # incorrect dimensions later.
            self.T = np.ones(standardize_sizes(sizes))
        else:
            self.T = T.copy()
    ...
    def copy(self):
        ...
        """
        pot = DiscretePotential(self.domain, self.sizes, self.T)
        pot.observed_domain = self.observed_domain.copy()
        return pot
    ...
    def arithmatic(self, pot, op='+'):
        ...
        """
        # len(self.sizes) == Number of nodes in this clique
        if len(self.sizes) != 1:
            pos = []
            for i in pot.domain:
                pos.append(self.domain.tolist().index(i))

            pot.sizes = np.array(pot.sizes,dtype='int') # HACK
            sz = np.ones(len(self.domain), dtype='int')
            sz[pos] = pot.sizes
        ...
        # This line is essential to correctly reshaping the potentials.
    ...
    def enter_evidence(self, evidence):
        ...
        self.observed_domain = np.array(odom)
        odom_index =  []
        for i in odom:
            if i in self.domain:
                odom_index.append(self.domain.tolist().index(i))
